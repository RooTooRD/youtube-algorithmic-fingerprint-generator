...
class AccountRecord(Base):
    ...
    __table_args__ = (Index("uq_accounts_profile_dir", "profile_dir", unique=True),)
    ...
    lease_acquired_at: Mapped[dt.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class Run(Base):
    __tablename__ = "runs"
    __table_args__ = (Index("uq_run_experiment_arm_rep", "experiment_id", "arm", "repetition", unique=True),)
...
class Video(Base):
    ...
    captions_available: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    metadata_status: Mapped[str] = mapped_column(String(16), default="pending", index=True)
    metadata_error: Mapped[str | None] = mapped_column(Text, nullable=True)
    transcript_status: Mapped[str] = mapped_column(String(16), default="pending", index=True)
    ...
    transcript_enriched_at: Mapped[dt.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
